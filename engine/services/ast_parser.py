...
class ASTParser:
    ...
    def parse_file(self, filepath: str, language: str) -> list:
        ...
        if language == "python":
            ...
            return self._parse_cobol(filepath)
        elif language == "fortran":
            return self._parse_fortran(filepath)
        elif language in ("pli", "rpg"):
    ...
    def _parse_cobol(self, filepath: str) -> list:
        """Parse COBOL files using regex-based extraction."""
        nodes = []
        try:
            with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                source = f.read()
            ...
            # Extract PROGRAM-ID
            prog_pattern = r"PROGRAM-ID\.\s+(\S+)"
            for match in re.finditer(prog_pattern, source, re.IGNORECASE):
                line_num = source[: match.start()].count("\n") + 1
                nodes.append({
                    "id": f"{filepath}:{match.group(1)}",
                    "label": match.group(1),
            ...
            # Extract SECTION names (e.g. PROCEDURE DIVISION, DATA DIVISION, custom sections)
            section_pattern = r"^\s{6,}\s*(\S[\w-]+)\s+SECTION\s*\."
            for match in re.finditer(section_pattern, source, re.MULTILINE | re.IGNORECASE):
                line_num = source[: match.start()].count("\n") + 1
                nodes.append({
                    "id": f"{filepath}:section:{match.group(1)}",
                    "label": f"{match.group(1)} SECTION",
                    "type": "function",
                    "file": filepath,
                    "line_start": line_num,
                    "language": "cobol",
            ...
            # Extract PARAGRAPH names (labels followed by a period at start of line in Area A)
            para_pattern = r"^       (\w[\w-]+)\.\s*$"
            for match in re.finditer(para_pattern, source, re.MULTILINE):
            ...
            # Extract PERFORM calls (function calls in COBOL)
            perform_pattern = r"PERFORM\s+([\w-]+)"
            calls = []
            for match in re.finditer(perform_pattern, source, re.IGNORECASE):
            ...
            # Extract COPY (copybook imports)
            copy_pattern = r"COPY\s+([\w-]+)"
            for match in re.finditer(copy_pattern, source, re.IGNORECASE):
                nodes.append({
                    "id": f"{filepath}:copy:{match.group(1)}",
                    "label": match.group(1),
                    "type": "import",
                    "file": filepath,
                    "language": "cobol",
            ...
            # Attach calls to the program node if it exists
            if nodes and calls:
                for n in nodes:
                    if n.get("subtype") == "program":
                        n["calls"] = list(set(calls))
                        break

            # Estimate COBOL-specific complexity
            ...
            for n in nodes:
                if n.get("subtype") == "program":
                    n["complexity"] = 1 + if_count + evaluate_count + goto_count
                    n["metrics"] = {
                        "loc": len(lines),
    ...
    def _parse_fortran(self, filepath: str) -> list:
        """Parse Fortran files using regex-based extraction."""
        nodes = []
        try:
            with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                source = f.read()
            ...
            # Extract PROGRAM
            prog_pattern = r"(?i)^\s*PROGRAM\s+(\w+)"
            for match in re.finditer(prog_pattern, source, re.MULTILINE):
                line_num = source[: match.start()].count("\n") + 1
                nodes.append({
                    "id": f"{filepath}:{match.group(1)}",
                    "label": match.group(1),
            ...
            # Extract SUBROUTINE
            sub_pattern = r"(?i)^\s*SUBROUTINE\s+(\w+)\s*\("
            for match in re.finditer(sub_pattern, source, re.MULTILINE):
                line_num = source[: match.start()].count("\n") + 1
                nodes.append({
                    "id": f"{filepath}:{match.group(1)}",
                    "label": f"{match.group(1)}()",
            ...
            # Extract FUNCTION
            func_pattern = r"(?i)^\s*(?:\w+\s+)?FUNCTION\s+(\w+)\s*\("
            ...
            # Extract MODULE
            mod_pattern = r"(?i)^\s*MODULE\s+(\w+)"
            for match in re.finditer(mod_pattern, source, re.MULTILINE):
                name = match.group(1)
                if name.upper() != "PROCEDURE":
                    line_num = source[: match.start()].count("\n") + 1
                    nodes.append({
                        "id": f"{filepath}:{name}",
                        "label": name,
            ...
            call_pattern = r"(?i)\bCALL\s+(\w+)"
            for match in re.finditer(call_pattern, source):
                nodes.append({
                    "id": f"{filepath}:call:{match.group(1)}",
                    "label": match.group(1),
                    "type": "import",
                    "file": filepath,
                    "language": "fortran",
            ...
            # Extract USE statements (module imports)
            use_pattern = r"(?i)^\s*USE\s+(\w+)"
            for match in re.finditer(use_pattern, source, re.MULTILINE):
                nodes.append({
                    "id": f"{filepath}:use:{match.group(1)}",
                    "label": match.group(1),
                    "type": "import",
                    "file": filepath,
                    "language": "fortran",
